...
| `php bin/import_wordpress.php --url https://example.com` | Imports posts and categories from a WordPress site via the REST API. Pass `--post <slug>` to import a single post, `--force` to re-import existing posts, `--auth username:app-password` to include drafts, and `-v` for verbose output. |
| `php bin/import_wordpress_xml.php --file export.xml` | Imports posts, categories, tags, and comments from a WordPress WXR export file. Drafts are included automatically. Pass `--post <slug>` to import a single post, `--force` to re-import existing posts, `--skip-images` to skip image downloading, and `-v` for verbose output. |
